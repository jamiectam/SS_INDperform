...
wss.long.m_all<-test_interaction(init_tbl=wss.long.data_init, mod_tbl=wss.long.calc, interactions=wss.long.it)

write_csv(wss.long.m_all, "wss.long.m_all_coli.csv")
save(wss.long.m_all, file="wss.long.m_all_coli.RData")
...
wss.short.glm_summary_coli<-allSummary
save(wss.short.glm_summary_coli,file='wss.short.glm_summary_coli.RData')
write_csv(wss.short.glm_summary_coli,'wss.short.glm_summary.csv_coli')
```
...
ess.short.glm_summary_coli<-allSummary
save (ess.short.glm_summary_coli,file='ess.short.glm_summary_coli.RData')
write_csv(ess.short.glm_summary_coli,'ess.short.glm_summary_coli.csv')
...
dri <- c(rep(c ("MAFA1", "MAFA2", 
                           ...
                           "Air_Temperature","AMO", "CIL_Volume", "Ice_Volume",
                           "NAO_Winter", "Salinity", "Stratification", "Bottom_Temp"), 18))
...
bestList <- allSummary  #[!is.na(allSummary$edf) &
                        # allSummary$edf > 1 &
                        # allSummary$Pvalue < 0.05,]
ess.long.glm_summary_coli<-allSummary
save(ess.long.glm_summary_coli,file='ess.long.glm_summary.RData')
write_csv(ess.long.glm_summary_coli,'ess.long.glm_summary.csv')
...
bestList <- allSummary  #[!is.na(allSummary$edf) &
                        # allSummary$edf > 1 &
                        # allSummary$Pvalue < 0.05,]
wss.long.glm_summary_coli<-allSummary
save (wss.long.glm_summary_coli,file='wss.long.glm_summary_coli.RData')
write_csv(wss.long.glm_summary_coli,'wss.long.glm_summary_coli.csv')
...
#load GAM data
load("ess.long.m_all_coli.RData")
load("wss.long.m_all_coli.RData")
load("ess.short.m_all_coli.RData")
load("wss.short.m_all_coli.RData")

##set up GLM and GAM data
#glm table as tibble
ess.long_glm_summary<-as_tibble(ess.long.glm_summary_coli)
ess.short_glm_summary<-as_tibble(ess.short.glm_summary_coli)
wss.long_glm_summary<-as_tibble(wss.long.glm_summary_coli)
wss.short_glm_summary<-as_tibble(wss.short.glm_summary_coli)

#add combo column to GLM tibble
...
#add combo column to GAM tibble
ess.long.m_all_tot<-ess.long.m_all %>% unite("combo", ind:press, sep="-", remove=FALSE)
ess.short.m_all_tot<-ess.short.m_all %>% unite("combo", ind:press, sep="-", remove=FALSE)
wss.long.m_all_tot<-wss.long.m_all %>% unite("combo", ind:press, sep="-", remove=FALSE)
wss.short.m_all_tot<-wss.short.m_all %>% unite("combo", ind:press, sep="-", remove=FALSE)

# join tibbles by combo
...
save(ess.long_gam_glm, file="ess.long_gam_glm_coli.RData")
save(ess.short_gam_glm, file="ess.short_gam_glm_coli.RData")
save(wss.long_gam_glm, file="wss.long_gam_glm_coli.RData")
save(wss.short_gam_glm, file="wss.short_gam_glm_coli.RData")

write_csv(ess.long_gam_glm, "ess.long_gam_glm_coli.csv")
write_csv(ess.short_gam_glm, "ess.short_gam_glm_coli.csv")
write_csv(wss.long_gam_glm, "wss.long_gam_glm_coli.csv")
write_csv(wss.short_gam_glm, "wss.short_gam_glm_coli.csv")
```
...
load("ess.long_gam_glm_coli.RData")
load("ess.short_gam_glm_coli.RData")
load("wss.short_gam_glm_coli.RData")
load("wss.long_gam_glm_coli.RData")
...
ess.long.l_fits<-ess.long_gam_glm %>%
                dplyr::filter(glm_Pvalue < 0.05,na.rm=TRUE)%>%
                dplyr::filter(glm_rsq>0.19) %>%
                 dplyr::filter(is.na(id))#%>% select(combo)
...
wss.long.l_fits<-wss.long_gam_glm %>%
                dplyr::filter(glm_Pvalue < 0.05,na.rm=TRUE)%>%
                dplyr::filter(glm_rsq>0.19) %>%
                 dplyr::filter(is.na(id))#%>% select(combo)
                
ess.short.l_fits_combo<-ess.short_gam_glm %>%
                dplyr::filter(glm_Pvalue < 0.05,na.rm=TRUE)%>%
                dplyr::filter(glm_rsq>0.19) %>%
                dplyr::filter(is.na(id)) %>% select(combo)
              

ess.long.l_fits_combo<-ess.long_gam_glm %>%
                dplyr::filter(glm_Pvalue < 0.05,na.rm=TRUE)%>%
                dplyr::filter(glm_rsq>0.19) %>%
                 dplyr::filter(is.na(id)) %>% select(combo)
               

wss.short.l_fits_combo<-wss.short_gam_glm %>%
                dplyr::filter(glm_Pvalue < 0.05, na.rm=TRUE)%>%
                 dplyr::filter(glm_rsq>0.019) %>%
                 dplyr::filter(is.na(id)) %>% select(combo)
               
wss.long.l_fits_combo<-wss.long_gam_glm %>%
                dplyr::filter(glm_Pvalue < 0.05,na.rm=TRUE)%>%
                dplyr::filter(glm_rsq>0.19) %>%
                 dplyr::filter(is.na(id)) %>% select(combo)

SS.l_fits_combo<-cbind.fill(ess.short.l_fits_combo, ess.long.l_fits_combo, wss.short.l_fits_combo, wss.long.l_fits_combo)
SS.l_fits<-cbind.fill(ess.short.l_fits, ess.long.l_fits, wss.short.l_fits, wss.long.l_fits)
SS.names<-c("ess.short", "ess.long", "wss.short", "wss.long")
colnames(SS.l_fits)<-SS.names
colnames(SS.l_fits_combo)<-SS.names

write_csv(SS.l_fits, "SS.l_fits_coli.csv")
write_csv(SS.l_fits, "SS.l_fits_combo_coli.csv")
...
ess.long.nl_fits<-ess.long_gam_glm %>%
                 dplyr::filter(!is.na(id)) %>%
                  dplyr::filter(p_val<0.05) %>% dplyr::filter(r_sq>0.19)#%>% select(combo)

wss.short.nl_fits<-wss.short_gam_glm %>%
                  dplyr::filter(!is.na(id)) %>%
                  dplyr::filter(p_val<0.05) %>% dplyr::filter(r_sq>0.19)#%>% select(combo)

wss.long.nl_fits<-wss.long_gam_glm %>%
                  dplyr::filter(!is.na(id)) %>%
                  dplyr::filter(p_val<0.05) %>% dplyr::filter(r_sq>0.19)#%>% select(combo)


ess.short.nl_fits_combo<-ess.short_gam_glm %>%
                  dplyr::filter(!is.na(id)) %>%
                  dplyr::filter(p_val<0.05) %>% dplyr::filter(r_sq>0.19)%>% select(combo)

ess.long.nl_fits_combo<-ess.long_gam_glm %>%
                 dplyr::filter(!is.na(id)) %>%
                  dplyr::filter(p_val<0.05) %>% dplyr::filter(r_sq>0.19)%>% select(combo)

wss.short.nl_fits_combo<-wss.short_gam_glm %>%
                  dplyr::filter(!is.na(id)) %>%
                  dplyr::filter(p_val<0.05) %>% dplyr::filter(r_sq>0.19)%>% select(combo)

wss.long.nl_fits_combo<-wss.long_gam_glm %>%
                  dplyr::filter(!is.na(id)) %>%
                  dplyr::filter(p_val<0.05) %>% dplyr::filter(r_sq>0.19)%>% select(combo)
...
SS.nl_fits_combo<-cbind.fill(ess.short.nl_fits_combo, ess.long.nl_fits_combo, wss.short.nl_fits_combo, wss.long.nl_fits_combo)
SS.nl_fits<-cbind.fill(ess.short.nl_fits, ess.long.nl_fits, wss.short.nl_fits, wss.long.nl_fits)

SS.names<-c("ess.short", "ess.long", "wss.short", "wss.long")
colnames(SS.nl_fits)<-SS.names
colnames(SS.nl_fits_combo)<-SS.names

 write_csv(SS.nl_fits, "SS.nl_fits_coli.csv")
  write_csv(SS.nl_fits, "SS.nl_fits_combo_coli.csv")
```
