...
wss.short.pressures<-short.pressures2 %>%
  dplyr::filter(ID=="WSS", YEAR >=1999) %>%
  dplyr::select(-YEAR, -ID) %>%
  as_tibble()


wss.short.year<-short.pressures2 %>%
  dplyr::filter(ID=="WSS", YEAR>=1999) %>%
  dplyr::select(YEAR)
...
wss.long.pressures<-long.pressures2 %>% 
  dplyr::filter(ID=="WSS") %>%
  dplyr::select(-YEAR, -ID) %>%
  as_tibble()

wss.long.year<-long.pressures2 %>%
  dplyr::filter(ID=="WSS") %>%
  dplyr::select(YEAR)
...
write_csv(ess.long.m_all, "ess.long.m_all.csv")
save(ess.long.m_all, file="ess.long.m_all.RData")
```
...
ess.short.pressures<-short.pressures2 %>% 
    dplyr::slice(-98) %>%
  dplyr::filter(ID=="ESS", YEAR>=1999) %>%
  dplyr::select(-YEAR, -ID)


ess.short.year<-short.pressures2 %>%
  dplyr::slice(-98) %>%  
  dplyr::filter(ID=="ESS", YEAR>=1999) %>%
  dplyr::select(YEAR)
